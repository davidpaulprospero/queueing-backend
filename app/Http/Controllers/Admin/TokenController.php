...
class TokenController extends Controller
{
    ...
    public function handleCallButton(Request $request)
    {
        // Create an array with call record data
        $callRecordData = [
            'name' => $request->input('name'),
            'message' => $request->input('message'),
            // Add other relevant data for the call record
        ];

        // Generate a unique filename for the JSON file (e.g., timestamp or UUID)
        $filename = time() . '.json';

        // Save the call record data as JSON in the specified file
        Storage::put('call_records/' . $filename, json_encode($callRecordData));

        // Broadcast the event to notify the frontend
        broadcast(new NewCallRecord($callRecordData))->toOthers();
    ...
    public function current(Request $request)
    {
        ...
        $tokens = Token::where('status', '0')
            ->orderBy('is_vip', 'DESC')
            ->orderBy('id', 'ASC')
            ->with('transactionType') // Add this line to eager load the transaction type
            ->get();
    ...
    public function report(Request $request)
    {
        ...
        return view('backend.admin.token.report', compact('counters', 'departments', 'officers', 'transactionTypes'));
    }
    ...
    public function reportData(Request $request)
    {
        ...
        if (empty($search)) {
            ...
            $tokensProccess = Token::where(function ($query)  use ($search, $transactionTypes) {
                ...
                if (!empty($search['transaction_type'])) {
                    $query->where('transaction_type_id', '=', $search['transaction_type']);
                }
