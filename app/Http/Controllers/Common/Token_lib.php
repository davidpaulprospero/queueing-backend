...
{ 
    ...
    public function newToken($department_id = null, $counter_id = null, $transaction_type_id = null)
    {  
        @date_default_timezone_set(session('app.timezone')); 

        $lastToken = Token::whereDate('created_at', date("Y-m-d"))
        ->where(function ($query) use ($department_id, $counter_id, $transaction_type_id) {
        ...
        if (empty($lastToken))
        {
            $token = $prefixVIP.$prefixDepartment.'000';
        }
        else
        {
        	if (empty($is_vip))
        	{
	            $prefix = mb_substr($lastToken, 2, 1);
