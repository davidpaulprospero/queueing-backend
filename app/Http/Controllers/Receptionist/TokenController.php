...
class TokenController extends Controller
{
    ...
    public function tokenAutoView()
    {
        ...
        if ($display->display == 5) {
            ...
            ->with('department.transactionTypes')
        ...
        $tokens = Token::select('token.id', 'token.counter_id', 'token.token_no', 'token.created_at', 'transaction_types.name AS transaction_type', DB::raw('COUNT(*) AS position'))
        ->join('token as t2', function ($join) {
            $join->on('token.counter_id', '=', 't2.counter_id')
                ->where('token.id', '>=', 't2.id');
        })
        ->join('transaction_types', 'token.transaction_type_id', '=', 'transaction_types.id')
        ->where('token.status', '=', 0)
        ->groupBy('token.id', 'token.counter_id', 'token.token_no', 'token.created_at', 'transaction_types.name')
        ->orderBy('token.counter_id', 'ASC')
        ->orderBy('position', 'ASC')
        ->get();

            foreach ($tokens as $token) {
                $transactionType = TransactionType::find($token->transaction_type_id);
                $token->transaction_type = $transactionType ? $transactionType->name : ''; // Set the transaction type name or an empty string if not found
            }
        
            $transactionTypes = collect();
            // foreach ($departmentList as $department) {
            //     $transactionTypes = $transactionTypes->concat($department->transactionTypes);
            // }

    
        return view('backend.receptionist.token.auto', compact('display', 'departmentList', 'tokens', 'transactionTypes'));
    }
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                if (!empty($settings)) { 
                    ...
                    $key = strtoupper($transactionType->key);
                    $counter = Counter::where('id', $min['counter_id'])->first();
                    $counterPrefix = strtoupper(substr($counter->name, 0, 1));
                    
                    $saveToken = [
                        'token_no' => $key . $counterPrefix . (new Token_lib)->newToken($request->department_id, $request->counter_id, $request->transaction_type_id),
